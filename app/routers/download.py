...
async def upload_file(
    ...
    if not file_path.exists():
        return JSONResponse(
            {uuid: "The file wasn't found on the disk"}, status_code=status.HTTP_404_NOT_FOUND
        )
    return StreamingResponse(
        stream_file(file_path),
        media_type=file.enctype,
        headers={"filename": file.name, "file_size": str(file.size), 'transfer-encoding': 'chunked',},
    )
